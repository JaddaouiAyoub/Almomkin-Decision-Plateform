...
export default async function EditCasePage({
  ...
  async function handleSubmit(formData: FormData) {
    ...
    await updateStudyCase(id, { title, isActive, order, newInformation, contentA, contentB });
    redirect("/admin/cases");
  }
      ...
      <form action={handleSubmit} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100">
          <div className="space-y-2 md:col-span-2">
            ...
            <textarea id="newInformation" name="newInformation" required defaultValue={studyCase.newInformation || ""} rows={4} className="w-full px-4 py-3 bg-amber-50/40 border border-amber-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500/50 resize-y" placeholder="Information révélée après l'analyse..." />
          </div>
          {/* Group A Content */}
